...
fn save2html(discussion_body: &str, discussion_number: &u64,
  ...
  render_html(save_html_file_path, context,
              discussion_number.to_string().as_str(), true)?;
  Ok(())
}

fn render_html(save_html_file_path: &str,
               mut context: Context, save_html_file_name: &str,
               save_to_index: bool) -> Result<()> {
  ...
  if save_to_index {
    let index_html_path = PathBuf::from(save_html_file_path).join("post.html");
    println!("Saving html file to {:?}", index_html_path);
...
pub fn from_markdown_file(markdown_file_path: &str,
                          save_html_file_path: &str,
                          save_to_index: bool) -> Result<()> {
  ...
  render_html(save_html_file_path, context, number, save_to_index)
}
...
pub fn from_markdown_dir(markdown_dir_path: &str, save_html_file_path: &str) -> Result<()> {
  for entry in fs::read_dir(markdown_dir_path)? {
    ...
    from_markdown_file(&path, save_html_file_path, false)?
  };
  Ok(())
}
...
pub fn render_post_list(save_html_file_path: &str) -> Result<()> {
  let mut context = Context::new();
  let post_item_list = PostItemList::new();
  context.insert("post_items", &post_item_list.posts);
  context.insert("nav_links", &get_nav_link());
  let tera = match Tera::new("resources/templates/**/*.html") {
    Ok(tera) => tera,
    Err(e) => return Err(anyhow!("{}", e)),
  };
  Ok(fs::write(PathBuf::from(save_html_file_path).join("archive.html"),
               tera.render("archive.html", &context)?)?)
}
#[cfg(test)]
mod tests {
  ...
  fn test_from_md() {
    from_markdown_file(r"C:\Users\26216\code\github\ttdly.github.io\posts\12.md",
                       r"D:\Code\web\test_page", false).expect("aaaa");
    let mut options = CopyOptions::new(); // Initialize default values for CopyOptions
    options.copy_inside = true;
    options.overwrite = true;
    copy_items(&["resources/templates/assets"], r"D:\Code\web\test_page", &options)
  ...
  fn test_render_post_list() {
    super::render_post_list(r"D:\Code\web\test_page").expect("aaaa");
  }
}